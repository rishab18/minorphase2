...
from .forms import LoginForm, SignupForm,ForgotPasswordForm, ResetPasswordForm, UpdateProfileForm
...
@require_GET
def index(request):
    products = Product.objects.order_by('-Till')[:4];
    productsall = Product.objects.all();
    data = {'products' : products, 'all' : productsall}
...
def UpdateProfile(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            user = form.save();
        #return render(request, 'account/authentication/profile-edit.html');
    else:
        form = UpdateProfileForm(instance=request.user);
    data = {'form' : form }
    return render(request, 'account/authentication/profile-edit.html',data);
...
def home(request):
    f = ProductUploadForm()
    form1 = BidPostForm();
    form2 = CategorySearchForm()
    form3 = SearchProductForm()
    products = []
    for product in Product.objects.all():
        data={}
        data['product']=product
        if request.user in product.Followedby.all():
            data["status"] = 1
        else:
            data["status"] = 0
        products.append(data)
    return render(request, 'product/product_upload.html',{'form' : f, 'form1': form1, 'products' : products, 'form2': form2, 'form3': form3})
